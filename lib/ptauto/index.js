(function() {
	...
	const process = async (config) => {
		...
		const ondemandTopItems = config.get('ondemandTopItems');
		const ondemandTopPicksOnly = config.get('ondemandTopPicksOnly');

		let chno = config.get('chno');
		if (chno !== false) chno = +chno;
		const ondemandTopItemsEnabled = ondemandTopItems !== false && ondemandTopItems !== 0 && ondemandTopItems !== '0';
		const parsedOndemandTopItemsLimit = parseInt(ondemandTopItems, 10);
		const ondemandTopItemsLimit = Number.isFinite(parsedOndemandTopItemsLimit) && parsedOndemandTopItemsLimit > 0
			? parsedOndemandTopItemsLimit
			: 1;
		const ondemandTopPicksOnlyEnabled = ondemandTopPicksOnly === true || ondemandTopPicksOnly === 'true' || ondemandTopPicksOnly === 1 || ondemandTopPicksOnly === '1';
			...
			try {
				...
				if (config.get('ondemand')) {
					...
					const res = await ondemand.generateM3U8(config, region, bootData, {
						topPicksOnly: ondemandTopPicksOnlyEnabled,
						topItemsOnly: ondemandTopItemsEnabled,
						topItemsLimit: ondemandTopItemsLimit
					});
