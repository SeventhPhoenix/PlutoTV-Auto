(function() {
	...
	const isTopPicksCategory = (category) => {
		const name = `${category?.name || ''}`.toLowerCase();
		const normalized = name.replace(/[^a-z0-9]/g, '');
		return normalized.includes('toppick') || normalized.includes('topmovies') || normalized.includes('featured');
	}

	// TODO: consolidate this and the life playlist generation code
	const generateM3U8 = async (config, region, bootData, options = {}) => {
		const xTvgUrl = config.get('xTvgUrl');
		const topPicksOnly = !!options.topPicksOnly;
		const topItemsOnly = !!options.topItemsOnly;
		const parsedTopItemsLimit = parseInt(options.topItemsLimit, 10);
		const topItemsLimit = Number.isFinite(parsedTopItemsLimit) && parsedTopItemsLimit > 0 ? parsedTopItemsLimit : 1;
		let cache = {};
		let newCache = {};
		let numChannels = 0;
		let chNo = 9000;
		...
		let categories = categoriesList.categories || [];
		if (topPicksOnly) {
			const topPicksCategories = categories.filter(isTopPicksCategory);
			categories = topPicksCategories.length ? topPicksCategories : (categories[0] ? [categories[0]] : []);
		}
		...
		for (let i = 0; i < categories.length; i++) {
			...
			for (let j = 0; j < items.items.length; j++) {
				...
				numChannels++;
				numAddedFromCategory++;

				newCache[`${id}-${region}`].stream_url = url;
				if (topItemsOnly && numAddedFromCategory >= topItemsLimit) break;
			}
			if (cacheDirty) try {
				cacheDirty = false;
