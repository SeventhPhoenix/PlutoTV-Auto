(function() {
	...
	const channels = async (region) => {
		...
		const resp = await axios.get(`https://${CHANNEL_HOST}/v2/guide/channels?channelIds=&offset=0&limit=1000&sort=number%3Aasc`, {headers});
	...
	const categories = async (region) => {
		...
		if (region) headers['X-Forwarded-For'] = region;
		const resp = await axios.get(`https://${CHANNEL_HOST}/v2/guide/categories`, {
			headers
		});
	...
	const timelines = async (region) => {
		...
		for (let offset = -1; offset < 24; offset += 4) {
			...
			for (let i = 0; i < channelIds.length; i += chunkSize) {
				...
				const clientTime = encodeURI(d.toISOString());
				const resp = await axios.get(`https://${CHANNEL_HOST}/v2/guide/timelines?start=${clientTime}&duration=240&channelIds=${chunks.join('%2C')}`, {headers});
